...
class A1Range(object):
    ...
    def __init__(
        self, start_cell: A1Cell = None, end_cell: A1Cell = None, sheet: str = None
    ):
    ...
    def __str__(self) -> str:
        ...
        return "{}{}:{}".format(
            f"'{self._sheet}'!" if self._sheet else "",
            self._range[0],
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, A1Range):
            ...
            return self._sheet == other._sheet and all(
                x == y for x, y in zip(self._range, other._range)
            )
